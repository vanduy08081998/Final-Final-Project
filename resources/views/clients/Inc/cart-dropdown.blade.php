<a class="navbar-tool-icon-box bg-secondary dropdown-toggle" href="shop-cart.html"><span
    class="navbar-tool-label cart-count">{{ count($cart) }}</span><i class="navbar-tool-icon ci-cart"></i></a><a class="navbar-tool-text"
    ...
    <div class="widget widget-cart px-3 pt-2 pb-3" style="width: 20rem;">
      ...
      <div class="d-flex flex-wrap justify-content-between align-items-center py-3">
        <div class="fs-sm me-2 py-2"><span class="text-muted">Tổng:</span><span
            class="text-accent fs-base ms-1">{{ number_format($totalprice) }}</span></div><a class="btn btn-outline-secondary btn-sm"
          href="{{ route('cart.cart-list') }}">Xem giỏ hàng<i class="ci-arrow-right ms-1 me-n1"></i></a>
      </div><a class="btn btn-primary btn-sm d-block w-100" href="{{ route('checkout.checkout-details') }}"><i
