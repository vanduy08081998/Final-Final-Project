                        ...
                        <div class="row">
                            ...
                            <div class="col-md-12 mb-3">
                                <div class="d-block user-comment">
                                    <div class="avt">
                                        <img src="{{ url('uploads/Users/' . $commentId->user->avatar) }}" alt="">
                                    </div>
                                    <strong class="name">{{ $commentId->user->name }}
                                    ...
                                    <div class="footer-comment">
                                        <em>{{ $commentId->created_at->diffForHumans() }}</em>
                                    </div>
                                ...
                                <div class="form-feedback">
                                    <form action="{{ route('comment.feedbackStore') }}" method="POST"
                                        enctype="multipart/form-data">
                                        @csrf
                                        <textarea name="comment_content" class="form-control" cols="3"
                                            rows="3">{{ '@' . $commentId->user->name . ' ' }}</textarea>
                                        @if ($parentId == 1)
                                            <input type="hidden" name="comment_parent_id" value="{{ $commentId->id }}">
                                        @else
                                            <input type="hidden" name="comment_parent_id"
                                                value="{{ $commentId->comment_parent_id }}">
                                        @endif
                                        <input type="hidden" name="comment_reply_id" value="{{ $commentId->id }}">
                                        <input type="hidden" name="comment_admin_feedback" value="0">
                                        <input type="hidden" name="comment_id_user" value="{{ Auth::user()->id }}">
                                        <input type="hidden" name="comment_id_product"
