                        ...
                        <div class="form-group">
                          ...
                          <select name="colors[]" id="product_color" class="form-control" multiple="multiple"
                            ...
                            @foreach (\App\Models\Color::all() as $color)
                              <option value="{{ $color->color_code }}" @if (in_array($color->color_code, json_decode($product->colors)))
                                selected
                            @endif>
                            {{ $color->color_name }}
                            </option>
                  ...
                  <div class="card-body">
                    <ul class="list-group notification-list">
                      ...
                      <li class="list-group-item">
                        Vận chuyển nhanh
                        <div class="status-toggle">
                          <input type="radio" id="flat_rate" name="shipping_type" value="flat_rate"
                            class="check" @if ($product->shipping_type == 'flat_rate') checked @endif>
                  ...
                  <div class="card-body">
                    ...
                    <ul class="list-group notification-list">
                      <li class="list-group-item">
                        ...
                        <div class="status-toggle">
                          <input type="radio" id="qt_total" name="show_hide_quantity" value="qt_total"
                            class="check" @if ($product->show_hide_quantity == 'qt_total') checked @endif>
                      ...
                      <li class="list-group-item">
                        ...
                        <div class="status-toggle">
                          <input type="radio" id="qt_stock" name="show_hide_quantity" value="qt_stock"
                            class="check" @if ($product->show_hide_quantity == 'qt_stock') checked @endif>
                    ...
                    <div class="form-group">
                      ...
                      <select name="id_flash_deal" class="form-control">
                        ...
                        @foreach ($flash_deals as $deal)
                          @if ($product->flash_deals()->first() != null)
                            <option value="{{ $deal->id }}" @if ($product->flash_deals()->first()->id == $deal->id) selected @endif>
                              {{ $deal->title }}</option>
                          @else
        ...
        <li class="list-group-item">
          ...
          <div class="status-toggle">
            <input type="checkbox" id="is-specification" name="has_specification" value="isAttribute"
              class="check" @if (count($product->specifications()->get()) > 0) checked @endif>
      ...
      <div class="row" id="show-hide-specification">
        <div class="col-md-12 col-sm-12 col-lg-12 col-12 col-xl-12">
          <div class="card">
            ...
            <div class="card-body">
              <div class="form-group">
                <select class="form-control" style="width:100%" name="choose_specification[]" id="choose_specification"
                  multiple="multiple">
                  @foreach (\App\Models\Attribute::all() as $attr)
                    <?php $fix_arr_specification = []; ?>
                ...
                <table class="table table-bordered mb-0 table-specifiation">
                  ...
                  <tbody>
                    @forelse (\App\Models\Specification::where('product_id', $product->id)->get() as $specification)
                      @foreach (\App\Models\Attribute::where('name', $specification->specifications)->get() as $fixed_attribute)
                        <tr>
                          <input type="hidden" name="choice_fixed_attribute[]" value="{{ $fixed_attribute->id }}">
                          <td>{{ $fixed_attribute->name }} <input type="hidden"
                              name="fixed_attribute_{{ $fixed_attribute->id }}"
                              value="{{ $fixed_attribute->name }}">
                          </td>
                          <td>
                            <select name="specifications_{{ $fixed_attribute->id }}" class="form-control">
                              @foreach (\App\Models\Variant::where('attribute_id', $fixed_attribute->id)->get() as $fixed_variant)
                                <option value="{{ $fixed_variant->name }}" @if ($fixed_variant->name == $specification->value) selected @endif>
                                  {{ $fixed_variant->name }}</option>
                              @endforeach
                            </select>
                          </td>
                        </tr>
