...
const singleModalShow = (field_id, id_render) => {
    $("#filemanagerModal").modal("show");
    $("#filemanagerModalMultiple").modal("hide");

    $("#singleFileManagerModal").attr(
        "src",
        `../../filemanager/dialog.php?field_id=${field_id}`
    );

    $("#filemanagerModal").on("hide.bs.modal", () => {
        let imageName = $(`#${field_id}`).val();
        let url = "../../uploads/";
        if (imageName == "") {
            $(`#${id_render}`).html(``);
        } else {
            $(`#${id_render}`).html(
                `<img src="${url}${imageName}" width="150px" height="150px" >`
            );
        }
    });
};

const multipleModalShow = (field_id, id_render) => {
    $("#filemanagerModal").modal("hide");
    $("#filemanagerModalMultiple").modal("show");

    $("#filemanagerModalMultiple").attr(
        "src",
        `../../filemanager/dialog.php?field_id=${field_id}`
    );

    $("#filemanagerModalMultiple").on("hide.bs.modal", () => {
        try {
            let images = $(`#${field_id}`).val();
            console.log("🚀 ~ file: app.js ~ line 511 ~ $ ~ images", images);
            let args = $.parseJSON(images);
            let url = "../../uploads/";
            let output = "";
            for (let i = 0; i < args.length; i++) {
                output += `<img src="${url}${args[i]}" width="150px" height="150px" >`;
            }
            $(`#${id_render}`).html(output);
        } catch (expression) {
            let imageName = $(`#${field_id}`).val();
            let url = "../../uploads/";
            $(`#${id_render}`).html(
